/*
 * grunt-doc-html
 * https://github.com/weblux/grunt-doc-html
 *
...
module.exports = function (grunt) {
  ...
  grunt.registerMultiTask('doc_html', 'A grunt plugin to generate documentation directly from patterns files', function () {
    ...
    if (versions.indexOf(currentVersion) !== -1) {
      grunt.fail.warn('HTML docs version ' + currentVersion + ' already exist.')
    }
    ...
    files.forEach(function (file, index) {
      ...
      var content = marked(file.datas.description)
      var demos = liveExample.create(file)
      if (demos.length) {
        demos.forEach(function (demo, index) {
          content = content.replace('{{data[' + index + ']}}', demo)
        })
      }
      content = assemble(header, file, index) + content + assemble(footer, file, index)
